...
extern "C"
{
    ...
    double calculatePi(int iterations)
    {
        ...
        /**
         * Определяем периодичность вывода из бибилиотеки, чтобы было понятно что она не зависла.
         * Если число итерация меньше 100000, то выводить промежуточно ничего не будет, все и так отработает быстро.
         * Если больше - будем выводить сообщение, но не на каждую итерацию, а так что всего будет 100 сообщений.
         * Для этого число итераций мы поделим на 100
         */
        ...
        for (int i = 0; i < iterations; ++i) // Цикл для проведения итераций
        {
            ...
            {
                std::cerr << "."; // Вывод точки для отслеживания прогресса вычислений
            }
        }

        std::cerr << "done" << std::endl; // Выод сообщения о завершении работы
